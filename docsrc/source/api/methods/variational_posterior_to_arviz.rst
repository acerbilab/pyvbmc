...
The returned xarray DataTree has one ``posterior`` group and one chain. In the
default form, every parameter is represented as a scalar variable over the
``chain`` and ``draw`` dimensions. Names default to ``x_0``, ...,
``x_{D-1}``, or can be supplied through ``var_names``.
...
:ref:`PyVBMC Example 9: Torch and JAX models and posterior exports` shows
posterior summaries for a fitted model.
