...
def test_init_no_lower_bound():
    parameter_transformer = ParameterTransformer(nvars=NVARS)
    assert np.all(np.isinf(parameter_transformer.lower_bound_orig))


def test_init_lower_bound():
    parameter_transformer = ParameterTransformer(
        nvars=NVARS, lower_bound=np.ones(NVARS)
    )
    assert np.all(parameter_transformer.lower_bound_orig == np.ones(NVARS))


def test_init_no_upper_bound():
    parameter_transformer = ParameterTransformer(nvars=NVARS)
    assert np.all(np.isinf(parameter_transformer.upper_bound_orig))


def test_init_upper_bound():
    parameter_transformer = ParameterTransformer(
        nvars=NVARS, upper_bound=np.ones(NVARS)
    )
    assert np.all(parameter_transformer.upper_bound_orig == np.ones(NVARS))
...
def test_init_type_1():
    parameter_transformer = ParameterTransformer(
        nvars=NVARS, lower_bound=np.ones(NVARS), upper_bound=np.ones(NVARS) * np.inf
    )
...
def test_init_type_2():
    parameter_transformer = ParameterTransformer(
        nvars=NVARS, lower_bound=np.ones(NVARS) * -np.inf, upper_bound=np.ones(NVARS)
    )
...
def test_init_type_3():
    parameter_transformer = ParameterTransformer(
        nvars=NVARS, lower_bound=np.ones(NVARS), upper_bound=np.ones(NVARS)*2
    )
