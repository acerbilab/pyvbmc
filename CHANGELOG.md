...
- The first GP fit of a run completes when the points of highest density in
  ...
  that length scale and the starting scale of the GP mean come from the
  whole training set. Later fits whose points of highest density share a
  value, which is common with `integer_vars`, take that lower bound from the
  whole training set as well, so their results differ from 1.0.4. Such fits
  print no `RuntimeWarning: divide by zero encountered in log`.
