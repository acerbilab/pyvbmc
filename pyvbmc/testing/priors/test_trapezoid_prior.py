...
from pyvbmc.priors import Trapezoid


def test_trapezoid_basic_pdf():
    D = np.random.randint(1, 21)
    prior = Trapezoid(0, 0.25, 0.75, 1, D=D)

    midpoint = np.full((1, D), 0.5)
    assert np.isclose(prior.pdf(midpoint), (4 / 3) ** D)


def test_trapezoid_random_pdf():
    D = np.random.randint(1, 21)
