...
matrix, skipped when only docs changed. An implementation that is rejected
or parked after evaluation leaves the working line and is kept for the
record on a branch named `retain/<topic>` (`retain/experimental-acquisitions`,
`retain/viqr-rqmc-nodes`), cut at the last commit that holds it and named
by the records that describe it; `feat-*` and `dev-*` branches are live
work. Pushes to `dev*` branches that touch
