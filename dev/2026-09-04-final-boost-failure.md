...
`student_D4` seed 19 is the only run of 280 that fails the papers'
usability criterion outright: ΔLML 1.33, gsKL 53.8, MMTV 0.44. It is not
a convergence failure. The trace and a bit-for-bit reproduction show:
...
centre is 2–4 posterior SD from the mode. Two signals mark the failure: the
...
data extent 10, 2, 2.3, 1.3 per dimension, and `m0` 8 nats below the
observed peak, against ω/extent 0.1–0.2 and 2–4 nats on all other 19
seeds; the log-ω spread over the 8 hyperparameter samples is 2–5 nats,
so some samples have essentially no curvature. Away from the data the GP
posterior mean reverts to that mean function, and the surrogate predicts a
log density about 15 nats too high in the empty parts of the plausible box
(GP mean −13.6 ± 2.0 at the giant component's centre, where the true
log density is −28.3).

Under such a surrogate a box-wide component gains about 9 nats of entropy
and loses about 8.5 nats of expected log joint, so the surrogate ELBO is
