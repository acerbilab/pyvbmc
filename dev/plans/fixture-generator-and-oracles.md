...
| `gp_nlZ` (added 2026-09-05, item 8) | on the bare rebuilt GP, `gp.log_likelihood(hyp_s, compute_grad=True)` per stored sample; on a copy with PyVBMC's hyperprior installed as `train_gp` does (`_gp_hyp`, then `fit`'s `df` fill and bound repair, in that order), `gp.log_posterior(hyp_s, compute_grad=True)` (gpyreg 1.1.0's public methods; until the evening of 2026-09-05 the private `__compute_nlZ`, negated, because the public ones raised with a gradient before 1.1.0; the switch left every reference bit-identical) | `lZ, lp (Ns,)`, `dlZ, dlp (Ns, hyp_N)` | values GP-solve class 1e-6 + 1e-10 abs; gradients 2e-2 (they go through the explicit inverse `Q = K⁻¹ − ααᵀ` and are the worst-conditioned quantity of all: per element Ubuntu moved them by 2.6e-6 on `cigar_D4_boosted` and 4.1e-4 on `corr_D5_warped`, macOS by 1.6e-6 on cigar, the values by 1e-8 to 9e-8; same-machine exactness comes from the dump gate) |
