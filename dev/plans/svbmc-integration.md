...
- **Validation and messages.** Reject an empty list, mismatched `D`,
  ...
  `I_sk`, `J_sjk` (finiteness only for runs marked stable, since the
  filters drop the others). Argument misuse warns through `warnings.warn`;
  progress goes through the `SVBMC` logger, as `VBMC` uses its own.
- **Fixtures.** No pickles. The thirty upstream posteriors are converted to
  ...
  The corpus is entirely unbounded D=2 (five of its thirty runs warped),
  so a supplementary set generated by short seeded VBMC runs adds D=1,
  bounded D=2 with a different plausible box per run, and D=3 mixing
...
pyvbmc/svbmc/LICENSE.txt          upstream BSD 3-Clause text (package data)
...
- 2026-09-11: three read-only Opus reviewers (implementation; tests and
  fixtures; docs, example and records) found no correctness defect in the
  numerics or the sampling. Folded in: the `logger` docstring said to
  lower the level to quieten a run (it must be raised); the fixture codec
  hard-coded the probit code, so a future logit fixture would have rebuilt
  with the wrong transform (the bounded-transform code is stored, all
  sidecars updated, and every written fixture is now rebuilt and compared
  by the generator); five of the thirty upstream posteriors carry a warp,
  which FIXTURES.md and the plan had denied; the references sidecar now
  records every argument of its recipe and the test passes them; an
  unfitted posterior (`stats is None`) gets a clear error; finiteness is
  required only of runs marked stable, since the filters drop the others;
  the `_balanced_counts` docstring wrongly claimed the same remainder
  scheme as the VP sampler, and its unreachable fallback is gone; the
  fractional `M_min` is rounded up in the error message; the loss
  sentinel is infinite; `overlay_corner_plot` lost a dead fallback for a
  stubbed `corner` and now widens each marginal panel to the tallest
  histogram (the first executed notebook clipped three of them);
  `find_init_bounds` documents the accepted layouts; the filters tests
  restore NumPy's global state; the statistical tests reseed the stacked
  object; the notebook's prose no longer claims the filters read the ELBO
  uncertainty, says the draws come from a run's reweighted components,
  gives the true log evidence (0) and log(1/2) as the reference for the
