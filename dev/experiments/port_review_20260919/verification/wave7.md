...
proposal and the column "PI" the PI's ruling of 2026-09-23, which took every
proposal as it stood ("as proposed"), the test notes worth acting on
included. PyVBMC 1.5 takes only bug fixes and unequivocal improvements, and
a fix may make an interface stricter but may not change behavior silently
(plan, "Decisions").
...
Made by four Opus fix agents on worktrees of their own
...
| W7-10 | `58d68a3d` | the parameter docstrings of `_initialize_full_elcbo` and `_neg_elcbo` |
...
Made on `dev-next` at `52ccdb63` with gpyreg 1.3.1, one process at a time;
the scripts are `scripts/wave7_A_w7_17_shared_coordinate_run.py` and
`scripts/wave7_A_w7_2_resume_effect.py`, their logs on the orchestrator's
machine (`dev/scripts/runs/LOCAL.md`, "Port correctness review").

- **W7-17.** A `D = 2` run capped at three iterations from ten starting
  points whose second coordinate is one value, and from twelve such points,
  evaluates the ten points of its initial design and stops at the first GP
  fit with L-BFGS-B's `KeyError: (-inf, -inf)` (`gaussian_process_train.py`,
  through gpyreg's `fit`); the same run from ten generic starting points
  completes. The PI ruled the refusal at construction (`8921845f`): with at
  least `fun_eval_start` starting points, a coordinate that takes one value
  across the first `fun_eval_start` of them raises `ValueError`; fewer
  points, whose design the plausible box completes, are taken.
- **W7-2.** A seeded run on a correlated Gaussian in `D = 3` keeps a warp at
  ...
  record that the fix writes, reproduce the uninterrupted run exactly, the
  ELBO equal at every iteration (final ELBO 1.62271, 0.00710 below the true
  log evidence; KL to the true posterior 0.00042). The same resume with the
  record that the code before the fix wrote, the `hyp_dict` of iteration 9,
  departs at iteration 11: the means of the GP hyperparameter samples there
  differ by up to 2.4, the final ELBO is 1.64087 (0.01106 above the true log
  evidence), the KL to the true posterior 0.00058, and the two final
  posteriors lie 0.0007 apart (Gaussian KL). The defect made a resume at a
  kept warp depart from the run it resumed; the fix makes it reproduce it.
