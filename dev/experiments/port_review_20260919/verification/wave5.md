...
| W5-20 | P7 comparison F11 | The sheet's entry on `qtrapz.m` says that the endpoint handling differs. `qtrapz(y)` is `sum(y) - 0.5*(y(1) + y(end))`, the trapezoid rule at unit spacing, and both sides multiply by the spacing afterwards | the sheet's entry is wrong (P7-10): the two agree bit for bit on short vectors and to 1e-14 on long ones, by the order of the sum | n/a | P7-10 | the entry corrected, in the verifier's words | as proposed |
