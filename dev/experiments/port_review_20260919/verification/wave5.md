...
the PI's ruling, empty until it is given. One row is ruled, W5-25, which the
PI decided while the verification was under way.
...
- The tests of four defects that two reviewers found independently are blind
  by construction: `test_sample_balance_*` with equal weights (W5-1), the two
  `samples=` tests of `kl_div` with one mean for every coordinate or `D = 1`
  (W5-9), `test_set_parameters_not_raw_negative_error` with a vector negative
  throughout (W5-10), the mode tests in `D = 2` under an identity transform
  (W5-11). Each fix brings the test that sees it.
...
reach; the mask of the acquisition, which stops the Nelder-Mead search on a
