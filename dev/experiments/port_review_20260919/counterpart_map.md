...
| `misc/setupvars_vbmc.m` | `pyvbmc/vbmc/vbmc.py:851: _init_optim_state` | P1b | ported | MATLAB commits `74b046e`/`f3e5d76` (2022) changed the bounded transform here; slice M checks it. |
...
| `misc/vptrain2real.m` | none; the call is commented out at `pyvbmc/vbmc/vbmc.py:1349`, `:1541` | P7 | unported | Only active for `vp.temperature` in `{2,3,4,5}`; identity at the default temperature. |
| `misc/warp_gpandvp_vbmc.m` | `pyvbmc/whitening/whitening.py:257: warp_gp_and_vp` | P8, O3 | partial | Rotoscaling only; the output-warping line is a comment (`whitening.py:300-302`). |
...
| `private/recompute_lcbmax.m` | `pyvbmc/vbmc/vbmc.py:2403: _recompute_lcb_max` | P1a | ported | |
...
| `private/vbmc_output.m` | `pyvbmc/vbmc/vbmc.py:3119: _create_result_dict` | none | partial | `overhead` is `NaN`; `rng_state` is a generator snapshot. See REPORT.md. |
| `private/vbmc_termination.m` | `pyvbmc/vbmc/vbmc.py:2091: _check_termination_conditions`, `:2208 _compute_reliability_index`, `:2266 _check_gp_sampling_stop`, `:2276 _is_gp_sampling_finished`, `:2331 _ensure_gp_sampling_history` | P1a | ported | |
| `private/vbmc_warmup.m` | `pyvbmc/vbmc/vbmc.py:1943: _check_warmup_end_conditions`, `:2025 _setup_vbmc_after_warmup` | P1a | ported | |
