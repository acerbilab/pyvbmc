...
def stray_tags(out, tags):
    ...
    Only the two things a case leaves are looked at, its ``<tag>.npz`` and
    its completion record, so that the campaign's own files (the manifest,
    the reports, the case list, the lock, a log or an error file) and the
    directories beside them (``records/``, ``slurm/``) are never mistaken
    for a run.
    """
    out = Path(out)
