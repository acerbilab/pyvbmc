...
signal stops and one whose task was killed outright; one, given a stand-in
site's details, has its tracked copies redacted. A two-run pool of the
smoke configuration, generated by ``svbmc_pool_run.py``, checks the
reading of pools, in either layout, and the gpyreg pin. The rest needs no
cell: the subsets, the summaries rebuilt from recorded results, and the
verification of a recreated baseline.
...
def test_an_interrupted_task_is_taken_over(campaign, tmp_path, slurm):
    ...
    assert contract.claim_path(out, task["tag"] + ".stale.800_1").exists()
...
def test_a_campaign_on_a_pool(pool, tmp_path):
    """The contract on a pool: the runs' hashes in the manifest, a changed
    run refused by the worker, the single-run rows from the pool records."""
    out = tmp_path / "campaign"
    arguments = ("--pool", str(pool), "--M", "2", "--repetitions", "1")
    arguments += ("--arms", "integrated", "--max-steps", str(MAX_STEPS))
    prepared = run_script("prepare", "--out", str(out), *arguments)
    assert prepared.returncode == 0, prepared.stdout + prepared.stderr
    manifest = contract.read_json(out / "manifest.json")
    assert manifest["kind"] == "run"
    assert manifest["pools"][0]["directory"] == str(pool.resolve())
    assert "baseline" not in manifest["identity"]["source"]["trees"]
    assert manifest["baseline"]["verification"] is None
    (line,) = run_script("cases", "--out", str(out)).stdout.splitlines()
    entry = manifest["conditions"][0]["entries"][0]
    npz = harness.run_files(entry["path"])[".npz"]
    kept = npz.read_bytes()
    npz.write_bytes(kept + b"\0")
    try:
        failed = run_script("worker", "--out", str(out), "--case", line)
    finally:
        npz.write_bytes(kept)
    assert failed.returncode == 1
    error = contract.error_path(out, line.split()[0]).read_text("utf-8")
    assert "differs from the SHA-256" in error
    for command in (("worker", "--case", line), ("verify",), ("assemble",)):
        result = run_script(command[0], "--out", str(out), *command[1:])
        assert result.returncode == 0, result.stdout + result.stderr
    assert not contract.error_path(out, line.split()[0]).exists()
    results = contract.read_json(out / "results.json")
    rows = results["single_run"][POOL_LABEL]
    records = [
        contract.read_json(
            Path(pool) / "records" / f"{e['name']}.complete.json"
        )
        for e in manifest["conditions"][0]["entries"]
    ]
