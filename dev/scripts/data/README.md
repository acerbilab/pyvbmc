...
one subject of Experiment 3 (uniform interval distribution, subject 2),
1512 trials, the "Bayesian timing" problem of the 2020 noisy-VBMC paper.
...
Ground truths regenerated by `dev/scripts/make_benchmark_truths.py` under
the suite's spline-trapezoidal priors, one `{name}.npz` plus `{name}.json`
per target (`timing`, `multisensory_s1`, `multisensory_s2`):
