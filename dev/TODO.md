...
about 7 h. Then night 2 (3f (ii), about 5 h), then the records of 3f (iii) and
the PR dev-next -> main. The dtype canary's one production follow-up, widening
float32/float16 constructor inputs (a strict xfail in test_vbmc_init.py today),
goes after the nights. Nothing numerical may land before the two nights are
done. The reference traces live only on this laptop under
